#=== illumina_gsamd_v1.R by PF Sullivan & Jie Song 12/2020
# GSA array processing for Illumina GSAMD v1.0
...
# input file: GSAMD-24v1-0_20011747_A1.zip
# output file: gsamd-v1-snp-informatics-2020-12.tsv.gz
# longleaf: /nas/depts/007/sullilab/shared/bioinf/arrays/illumina-gsamd-v1
...
library(data.table)
library(skimr)
setwd("/nas/depts/007/sullilab/shared/bioinf/arrays/illumina-gsamd-v1")

#=== unzip array manifests 
# GSAMD-24v1-0_20011747_A1.zip
system2("unzip", args = c("GSAMD-24v1-0_20011747_A1.zip"))

#=== rename files (manual step)
system("mv GSAMD-24v1-0_20011747_A1.csv GSAMD-24v1-0_20011747_A1.hg19.csv")

#=== read 
# get read start and end lines
c <- "GSAMD-24v1-0_20011747_A1.hg19.csv"
...
# TO=/nas/depts/007/sullilab/shared/bioinf/arrays/illumina-gsamd-v1
# cd ${TO}
# srun -t 8:00:00 -p interact -N 1 -n 4 --mem=16g --pty /bin/bash
# module load bowtie/1.2.3
# gunzip gsamd-v1.fasta.gz
# bowtie -m 10000 -v 0 -a -f --suppress 5,6,7,8 /nas/depts/007/sullilab/shared/bin/bowtie/grch38_1kgmaj gsamd-v1.fasta gsamd-v1.hg38.txt
# bowtie -m 10000 -v 0 -a -f --suppress 5,6,7,8 /nas/depts/007/sullilab/shared/bin/bowtie/hg19_1kgmaj gsamd-v1.fasta gsamd-v1.hg19.txt
# gzip gsamd*
...
#=== bowtie output hg38
# reads processed: 700078
# reads with at least one reported alignment: 674196 (96.30%)
# reads that failed to align: 25872 (3.70%)
# Reported 2781593 alignments
#=== bowtie output hg19
# reads processed: 700078
# reads with at least one reported alignment: 674245 (96.31%)
# reads that failed to align: 25823 (3.69%)
# Reported 2811430 alignments

#=== continue processing (download gsamd-v1.hg38.txt.gz and gsamd-v1.hg19.txt.gz)
# rules: + strand bp=bowtieBP+51, - strand bp=bowtieBP. probe: bowtieBP to bowtieBP+50 (0-based)
# keep those present once
k <- fread("gsamd-v1.hg38.txt.gz") %>%
...
l <- fread("gsamd-v1.hg19.txt.gz") %>%
  mutate(V3 = paste0("chr", V3)) %>%    
...
#2: chr1 630077 630127  . -1 -1  .  0
#3: chr1 631712 631762  . -1 -1  .  0
#4: chr1 791532 791582  . -1 -1  .  0
...
t <- "gsamd-v1-snp-informatics-2020-12.tsv"
fwrite(u, file=t, sep="\t")
system2("gzip", args = c("-f", t))

# cleanup
rm(list=ls(pattern="^[a-z]"))
system("rm GSAMD-24v1-0_20011747_A1.hg19.csv gsamd-v1.fasta.gz")
system("rm -r tmp/")

#to verify the integrity
system("md5sum gsamd-v1-snp-informatics-2020-12.tsv.gz")
