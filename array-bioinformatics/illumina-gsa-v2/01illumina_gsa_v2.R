#=== illumina_gsa_v2.R by PF Sullivan & Jie Song 12/2020
# GSA array processing for Illumina GSA v2.0
...
# longleaf: /nas/depts/007/sullilab/shared/bioinf/arrays/illumina-gsa-v2

#=== initialize
library(tidyverse)
library(data.table)
library(skimr)
setwd("/nas/depts/007/sullilab/shared/bioinf/arrays/illumina-gsa-v2")

#=== download array manifests 
# grch38: GSA-24v2-0_A2.csv
#a <- "ftp://webdata2:webdata2@ussd-ftp.illumina.com/downloads/productfiles/global-screening-array/v2-0/infinium-global-screening-array-24-v2-0-a2-manifest-file-csv.zip"
#b <- word(string = a, start = -1, sep = "/")
#download.file(url = a, destfile = b)
#system2("unzip", args = c(b))

# grch37: GSA-24v2-0_A1.csv
#a <- "ftp://webdata2:webdata2@ussd-ftp.illumina.com/downloads/productfiles/global-screening-array/v2-0/gsa-24-v2-0-A1-manifest-file-csv.zip"
#b <- word(string = a, start = -1, sep = "/")
#download.file(url = a, destfile = b)
#system2("unzip", args = c(b))

#=== rename files (manual step)
# system("mv GSA-24v2-0_A2.csv GSA-24v2-0_A2.hg38.csv")
# system("mv GSA-24v2-0_A1.csv GSA-24v2-0_A1.hg19.csv")

#=== read hg38 version
# get read start and end lines
c <- "GSA-24v2-0_A2.hg38.csv"
...
# TO=/nas/depts/007/sullilab/shared/bioinf/arrays/illumina-gsa-v2
# cd ${TO}
# srun -t 8:00:00 -p interact -N 1 -n 4 --mem=16g --pty /bin/bash
# module load bowtie/1.2.3
# gunzip gsa-v2.fasta.gz
# bowtie -m 10000 -v 0 -a -f --suppress 5,6,7,8 /nas/depts/007/sullilab/shared/bin/bowtie/grch38_1kgmaj gsa-v2.fasta gsa-v2.hg38.txt
# bowtie -m 10000 -v 0 -a -f --suppress 5,6,7,8 /nas/depts/007/sullilab/shared/bin/bowtie/hg19_1kgmaj   gsa-v2.fasta gsa-v2.hg19.txt
# gzip gsa-v2*
...
#=== bowtie output hg38
# reads processed: 665608
# reads with at least one reported alignment: 639321 (96.05%)
# reads that failed to align: 26287 (3.95%)
# Reported 643853 alignments
#=== bowtie output hg19
# reads processed: 665608
# reads with at least one reported alignment: 639739 (96.11%)
# reads that failed to align: 25869 (3.89%)
# Reported 684339 alignments


#=== continue processing (download gsa-v2.hg38.txt.gz and gsa-v2.hg19.txt.gz)
# rules: + strand bp=bowtieBP+51, - strand bp=bowtieBP. probe: bowtieBP to bowtieBP+50 (0-based)
# keep those present once
k <- fread("gsa-v2.hg38.txt.gz") %>%
...
#1: chr1 630053 630103    .     -1     -1     .  0
#2: chr1 632828 632878    .     -1     -1     .  0
#3: chr1 633147 633197    .     -1     -1     .  0
#4: chr1 792461 792511    .     -1     -1     .  0
#5: chr1 817290 817340    .     -1     -1     .  0
#6: chr1 823656 823706 chr1 823669 823670 3.023  1
...
t <- "gsa-v2-snp-informatics-2020-12.tsv"
fwrite(u, file=t, sep="\t")
system2("gzip", args = c("-f", t))

# cleanup
rm(list=ls(pattern="^[a-z]"))
system("rm GSA-24v2-0_A2.hg38.csv GSA-24v2-0_A1.hg19.csv gsa-v2.fasta.gz")
system("rm -r tmp/")

#to verify the integrity
system("md5sum gsa-v2-snp-informatics-2020-12.tsv.gz")
