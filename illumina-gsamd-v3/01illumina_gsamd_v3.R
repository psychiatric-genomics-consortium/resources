#=== illumina_gsamd_v3.R by PF Sullivan & Jie Song 12/2020
# GSA array processing for Illumina GSAMD v3.0
...
# input file: GSAMD-24v3-0-EA_20034606_A1.zip
# output file: gsamd-v3-snp-informatics-2020-12.tsv.gz
# longleaf: /nas/depts/007/sullilab/shared/bioinf/arrays/illumina-gsamd-v3
...
library(data.table)
library(skimr)
setwd("/nas/depts/007/sullilab/shared/bioinf/arrays/illumina-gsamd-v3")

#=== unzip array manifests 
# GSAMD-24v3-0-EA_20034606_A1.csv
system2("unzip", args = c("GSAMD-24v3-0-EA_20034606_A1.zip"))

#=== rename files (manual step)
system("mv GSAMD-24v3-0-EA_20034606_A1.csv GSAMD-24v3-0-EA_20034606_A1.hg19.csv")

#=== read 
# get read start and end lines
c <- "GSAMD-24v3-0-EA_20034606_A1.hg19.csv"
...
# TO=/nas/depts/007/sullilab/shared/bioinf/arrays/illumina-gsamd-v3
# cd ${TO}
# srun -t 8:00:00 -p interact -N 1 -n 4 --mem=16g --pty /bin/bash
# module load bowtie/1.2.3
# gunzip gsamd-v3.fasta.gz
# bowtie -m 10000 -v 0 -a -f --suppress 5,6,7,8 /nas/depts/007/sullilab/shared/bin/bowtie/grch38_1kgmaj gsamd-v3.fasta gsamd-v3.hg38.txt
# bowtie -m 10000 -v 0 -a -f --suppress 5,6,7,8 /nas/depts/007/sullilab/shared/bin/bowtie/hg19_1kgmaj gsamd-v3.fasta gsamd-v3.hg19.txt
# gzip gsamd*
...
#=== bowtie output hg38
# reads processed: 730059
# reads with at least one reported alignment: 700774 (95.99%)
# reads that failed to align: 29283 (4.01%)
# Reported 744612 alignments
#=== bowtie output hg19
# reads processed: 730059
# reads with at least one reported alignment: 701191 (96.05%)
# reads that failed to align: 28866 (3.95%)
# Reported 787131 alignments


#=== continue processing (download gsamd-v3.hg38.txt.gz and gsamd-v3.hg19.txt.gz)
# rules: + strand bp=bowtieBP+51, - strand bp=bowtieBP. probe: bowtieBP to bowtieBP+50 (0-based)
# keep those present once
k <- fread("gsamd-v3.hg38.txt.gz") %>%
...
#3: chr1 632828 632878    .     -1     -1     .  0
#4: chr1 633147 633197    .     -1     -1     .  0
#5: chr1 792461 792511    .     -1     -1     .  0
#6: chr1 817290 817340    .     -1     -1     .  0
#7: chr1 823656 823706 chr1 823669 823670 3.023  1
#8: chr1 858952 859002 chr1 858961 858962 3.143  1
#9: chr1 866105 866155 chr1 866125 866126 3.156  1
#10: chr1 899399 899449 chr1 899412 899413 3.296  1
...
t <- "gsamd-v3-snp-informatics-2020-12.tsv"
fwrite(u, file=t, sep="\t")
system2("gzip", args = c("-f", t))

# cleanup
rm(list=ls(pattern="^[a-z]"))
system("rm GSAMD-24v3-0-EA_20034606_A1.hg19.csv gsamd-v3.fasta.gz")
system("rm -r tmp/")

#to verify the integrity
system("md5sum gsamd-v3-snp-informatics-2020-12.tsv.gz")
